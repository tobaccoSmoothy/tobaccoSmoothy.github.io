...
THREEx.KeyboardState.MODIFIERS = [
    'shift',
    'ctrl',
    'alt',
    'meta'
 ...
 */ THREEx.KeyboardState.prototype.eventMatches = function(event, keyDesc) {
    ...
    for(var i = 0; i < keys.length; i++){
        var key = keys[i];
        var pressed = false;
        if (key === 'shift') pressed = event.shiftKey ? true : false;
        else if (key === 'ctrl') pressed = event.ctrlKey ? true : false;
        else if (key === 'alt') pressed = event.altKey ? true : false;
        else if (key === 'meta') pressed = event.metaKey ? true : false;
        else if (aliasKeys.indexOf(key) !== -1) pressed = event.keyCode === aliases[key] ? true : false;
        else if (event.keyCode === key.toUpperCase().charCodeAt(0)) pressed = true;
        if (!pressed) return false;
    }
    return true;
};

//# sourceMappingURL=index.96c37def.js.map
